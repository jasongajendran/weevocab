...
app.post('/api/ask-bard', async (req, res) => {
  try {
    const { prompt, currentWord, mode } = req.body;
...
Always provide definitions, at least two lively relatable examples (e.g. school life, sports, weather, Scottish adventures), synonyms, and antonyms where relevant.`;
    ...
    if (mode === 'story') {
      systemInstruction += `\nTask: Generate a hilarious, engaging 2-to-3 paragraph short adventure story for Scottish kids aged 10-15 incorporating the requested vocabulary words. Highlight the words in **bold** and provide a mini glossary at the bottom.`;
    ...
    res.status(500).json({
      error: error.message || 'Failed to get response from Hamish the Bard. Check if GEMINI_API_KEY is configured.',
    });
  }
});
...
const PORT = 3000;
// Note: In Vite dev mode, Vite handles port 3000. In standalone node mode, this server starts.
